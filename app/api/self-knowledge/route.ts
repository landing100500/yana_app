...
export async function POST(request: NextRequest) {
  try {
    ...
    // Опционально: ответить только на один вопрос (1–16)
    let questionNumber: number | null = null;
    try {
      const body = await request.json().catch(() => ({}));
      if (typeof body.questionNumber === 'number' && body.questionNumber >= 1 && body.questionNumber <= 16) {
    ...
    // Предпочитаем основную натальную карту, иначе последнюю
