...
export function formatSectionMemoryHint(label: string, result: SectionChunksFetchResult): string {
  ...
  if (result.status === 'not_found') {
    return (
      ...
      `Не утверждай, что он подключён; опирайся на другие фрагменты и данные карты. ---\n`
    );
  }
  if (result.status === 'disabled') {
    return (
      `\n--- Статус области «${label}»: раздел «${result.sectionName}» есть в базе, но не подключён к агенту ` +
      `(enabled_for_agent=false). Скажи, что область не подключена администратором. ---\n`
    );
  }
  if (result.status === 'empty_rag') {
    return (
      `\n--- Статус области «${label}»: раздел «${result.sectionName}» подключён к агенту, ` +
      `но релевантные фрагменты для этого запроса не подобрались. Не говори, что области нет в памяти; ` +
      `используй данные натальной карты, расчётные транзиты и другие переданные блоки. ---\n`
    );
  }
  return '';
}
