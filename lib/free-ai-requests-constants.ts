...
/** Остаток бесплатных AI-запросов (SQL expression по таблице users). */
export const FREE_AI_REMAINING_SQL = `GREATEST(
  0,
  CAST(COALESCE(users.freeAiRequestsLimit, ${FREE_AI_REQUESTS_LIMIT}) AS SIGNED)
  - CAST(COALESCE(users.freeAiRequestsUsed, 0) AS SIGNED)
)`;
